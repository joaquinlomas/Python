...
#    c.1) que personaje tiene la linea de dialogo mas y menos extenso? (numero de palabras)
#    c.2) Que personaje tiene mas y menos dialogos?

# C.1 

contador = dict([i, 0] for i in diccionario.keys())         # Generacion de diccionario contador de palabras
...
for k in diccionario.keys():                                # Para cada personaje del diccionario.
    aux = []                                                # Generacion de lista auxiliar.
    for j in diccionario[k]:                                # Para cada dialogo de un personaje.
        aux.append(len(j.split()))                          # Guarda en la auxiliar la cantidad de palabras de cada dialogo de cada personaje.
    contador[k] = [max(aux),min(aux)]                       # En contador se guarda la cantidad maxima y minima de palabras en la lista auxiliar.

#print contador

maximo = max(contador[k][0] for k in contador.keys())       # Encuentra la cantidad de palabras de la linea de dialogo mas larga de todo el guion.
minimo = min(contador[k][1] for k in contador.keys())       # Encuentra la cantidad de palabras de la linea de dialogo mas corta de todo el guion.
dialogo_max = []                                            # Generacion de lista de personaje(s) que tiene(n) la linea de dialogo mas larga.
dialogo_min = []                                            # Generacion de lista de personaje(s) que tiene(n) la linea de dialogo mas corta.
for k in contador.keys():                                   # Para cada personaje,
    if contador[k][0] == maximo:                            # Si su dialogo mas largo tiene el largo maximo encontrado en todo el guion,
        dialogo_max.append(k)                               # entonces agrega este personaje a la lista de personajes con el dialogo mas largo.
...
#personaje = raw_input('Introduzca el nombre de un personaje: ')   # Opcion de ingreso de personaje por consola.

personaje='JARVIS'
interactuan_con_personaje = interactua_con(personaje,'guion2.txt')


print('Los personajes que interactuan con '+personaje+' son:'+ str(interactuan_con_personaje)) # Impresion de personajes relacionados.
