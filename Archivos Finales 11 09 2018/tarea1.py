...
from modulo import leer_archivo,interactua_con,listsum
...
numero_palabras = dict([i, 0] for i in diccionario.keys())  # Generacion de diccionario de cantidad de palabras

for k in contador.keys():                                   # Por cada key de contador (lista de personajes),
    numero_palabras[k]=listsum(contador[k])                 # suma la cantidad de paralabras totales de cada uno y guardalo en la lista numero_palabras.


dialogo_mas_extenso=max(numero_palabras.iteritems(), key=operator.itemgetter(1))[0]   # Funcion lee todo el diccionario y entrega el key (personaje) 
                                                                                      # donde este el value mayor (El que tenga el dialogo mas largo segun cant de palabras).
dialogo_menos_extenso=min(numero_palabras.iteritems(), key=operator.itemgetter(1))[0] # Funcion lee todo el diccionario y entrega el key (personaje) 
